...
NH_APP_KEY          = os.environ.get("NH_APP_KEY", "")
NH_APP_SECRET       = os.environ.get("NH_APP_SECRET", "")
NH_INITIAL_TOKEN    = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJ0b2tlbiIsImF1ZCI6IjQxMzc1YTg4LWFhZGQtNGYyNi04YmZhLTQxYzg2YzY5ODAzNyIsIm5iZiI6MTc4OTU2MjUzMCwiZ3JhbnRfdHlwZSI6IkNsaWVudCIsImlzcyI6InVub2d3IiwiZXhwIjoxNzg5NjQ4OTMwLCJpYXQiOjE3ODk1NjI1MzAsImp0aSI6IjlZWDZNSzZMWlA5NUpZTENOUzVUN1E0SFRDVzI3VVlOIn0.urXq42ACCNjQkfCa3n2x-R8DiDmnhC7Nezz7DESy9BkntYGHIr0OBYDPjVSHs0IQkZOr3P5MU3-CF8GIhY0iNQ"
...
# ─────────────────────────────────────────────
def get_nh_token():
    if NH_APP_KEY and NH_APP_SECRET:
        try:
            url = "https://api.nhplug.com:8443/oauth2/token"
            ...
            print(f"NH 토큰 재발급 실패: {e}")
    return NH_INITIAL_TOKEN

def get_fx_rates():
    rates = {"USDKRW": 1380.0, "NOKKRW": 130.0}
...
def build_message(us_data, no_data, fx):
    ...
    for d in us_data:
        ...
        eval_krw = p["current"] * fx["USDKRW"] * h["shares"]
        cost_krw = h["book_krw"]
        ...
        msg1_lines.append(
            f"[{ticker}] ${p['current']:.2f} ({pct(p['change_pct'])}) | 수익 {pct(ret_pct)}\n"
    ...
    for d in no_data:
        ...
        eval_krw = p["current"] * fx["NOKKRW"] * h["shares"]
        cost_krw = h["book_krw"]
        profit_krw = eval_krw - cost_krw
        ret_pct = (profit_krw / cost_krw * 100) if cost_krw else 0
        ...
        msg2_lines.append(
            f"[{clean_ticker}] {p['current']:.2f} NOK ({pct(p['change_pct'])}) | 수익 {pct(ret_pct)}\n"
            f"  평가 {eval_krw/10000:,.0f}만 ({money(profit_krw)})"
        )
...
def main():
    nh_token = get_nh_token()
    if nh_token:
        print("NH Access Token 준비 완료")
